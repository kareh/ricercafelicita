...
Inoltre, esistono persone religiose che hanno scelto di servire la Chiesa e di essere retribuite da essa per seguire una vocazione autentica. E tra esse molte sono persone in gamba, che nel mondo normale, se lo avessero seguito, avrebbero trovato successo lavorativo.
...
In generale, se consideriamo una unione di molteplici esseri, scriveremo $\mathcal{A}=A_1+A_2+\cdots$. Quando pensiamo l'anima dal punto di vista di $A_i$, scriveremo $\mathcal{A}_{A_i}$, o piu' semplicemente $\mathcal{A}_i$.
...
Con questa definizione, possiamo dire che un animo ama $\anima{A}$, quando si spende affinche' i desideri di ogni $A_i$ siano soddisfatti, dando piu' priorita' ai bisogni ed ai desideri non superflui che agli altri desideri.\\
...
\def\Io{\textrm{Io}}
   ...
   ..... TODO ....: qui di seguito da riscrivere. Veniva fatta una confusione tra Io e Se'. L'Anima come esposta in precedenza e' il Se', e in questa sezione veniva detto che e' anche Io. Bisogna avere chiaro che un essere e' sia Se' (stato) che Io (volonta'). Il se' e' il vissuto, lo stato, la condizione e la predisposizione e il sogno di un essere, e quindi e' piu' in linea con la definizione di anima data in precedenza. L'io e' cio' che muove l'essere, al fine di esaudire e soddisfare i suoi desideri e bisogni, richiedendo all'essere inevitabilmente a volte fatica e dolori piccoli o grandi. L'Io e' l'unione degli io. E come prima per l'Anima, si puo' pensare che $\Io_i$ e' l'$\Io$ come portato avanti dalla componente $\textrm{io}_i$. Dio e' esattamente l'Io di cui l'anima ha bisogno. E quindi, inizialmente solo in \emph{negativo}, Dio esiste, in quanto i bisogni e desideri dell'anima esistono. A volte l'Io proprio o altrui soddisfa pienamente i bisogni, a volte meno o carentemente. Nel primo caso, l'anima trova in quell'uomo o donna (in parte) Dio (vede un riflesso della Sua luce, si dice). Ad ogni modo, quanto piu' l'Anima trova in qualcuno o qualcuna Dio, tanto piu' in \emph{positivo} Dio esiste. .... TODO .....

    Siamo partiti in questo capitolo da un essere e la sua anima, poi abbiamo detto che se l'essere e' di natura empatica, la sua anima non parla solo di se stesso, ma parla anche degli altri esseri. Abbiamo anche incluso artificialmente la Natura come essere di cui la sua anima parla.

\def\esAi{\anima{A}_i}
\def\esAj{\anima{A}_j}
\def\esAE{\anima{A}_E}

    Abbiamo trattato l'Anima, come se fosse una entita' indipendente che agisce, tuttavia, essa e' un'aspetto dello stesso essere. E' l'essere che agisce, che parla di se stesso e degli altri e della Natura, in maniera consapevole ad egli stesso od inconscia. Quindi, $\anima{A}_{A_i}=A_1+A_2+\cdots+N$ e' l'essere in se', nella sua totalita'!, e non solo nel suo Io.
    

\begin{defn}
	\label{soulGodDefition}
    L'essere $\esAi$ e' Dio di se stesso perche' \emph{definisce} e \emph{determina} il bene per se' e il bene che lui e' per gli altri che ama. A priori, non e' Dio di qualsiasi essere, neanche di quelli che ama. A priori, se ama, e' Dio \emph{per se stesso}.

    $\esAi$ definisce il bene perche' lui in quanto Anima definisce cio' che e' piacere e cio' che e' dolore. Se e' ugualitario, tutti gli esseri che hanno una definizione uguale alla sua saranno inclusi nell'unione $\esAi$ e amati da lui. Se e' caritatevole, anche tutti gli esseri che non sono uguali a lui, ma almeno rispettano, non contravvenendo la sua definizione di essere e di amare, saranno pure inclusi nella sua Anima e amati.

    L'essere determina il bene, perche' spende ogni forza degli animi suoi per adempiere il suo amore, ovvero per soddisfare i desideri di ogni essere $a_j$ della sua unione.\\

    Quanto abbiamo detto per $\esAi$, sussiste per tutti gli esseri: ogni essere esistente e' Dio di se stesso. Come e' possibile allora pensare ad un solo Dio?

    Se $\esAi$ ama un altro essere, lo considera al pari di se stesso, e quindi, cosi' come lui e' Dio per se stesso, vedra' l'altro come Dio per se stesso. Quindi, per ogni essere $a_j$ dell'unione $\esAi=a_1+a_2+\cdots$ si puo' scrivere:
\begin{align*}
    \anima{A}_i \supseteq \anima{A}_j
\end{align*}
    intendendo che l'unione $\esAj$ e' tutta inclusa nell'unione $\esAi$. Nello specifico se $a_k$ e' parte dell'unione $\anima{A}_j$, allora sara' parte dell'unione $\anima{A}_i$, (ed e' anche ricorsivo\footnote{Ricorsivamente vuol dire che se $a_{m}$ e' parte di $\anima{A}_k$, che e' incluso in $\anima{A}_j$, che e' incluso in $\anima{A}_i$, allora $a_m$ sara' anche parte di $\anima{A}_i$ e cosi' per ogni catena di lunghezza qualsiasi.
}).

    Se anche $\anima{A}_j$ ama $\anima{A}_i$, allora ognuno comprende l'unione dell'altro e quindi le due anime sono uguali: $\anima{A}_i = \anima{A}_j$.\\

    Anche un qualsiasi essere $E$ non dell'unione $\esAi$ e' Dio di se stesso. Cosa succede allora? Ci sono tre casi:

    \begin{enumerate}

        \item    Se due esseri non si amano, saranno nemici, e ognuno tentera' di affermarsi come Dio.
...
Infine, se consideriamo $N$ come un'essere, possiamo ripetere per $N$ gli stessi discorsi fatti per $\esAi$, per $\esAj$, che e' un qualsiasi essere amato da $\esAi$ ed per E, essere che non ama $\esAi$. Quindi $\esAi$ puo' non amare $N$ e quindi dolersi della sua o altrui sorte fisica, non spendere energia fisica per migliorare la sorte, e considerare la Natura $N$ come matrigna e nemica. Oppure, $\esAi$ puo' amare $N$, accettare i limiti di essere un uomo (o donna) per se stesso $A_i$ e per ogni $A_j$ che ama, e puo' impegnarsi per tendere sempre a stare bene e in pace nella Natura. Viceversa, la Natura, essendo materia, ed essendo sempre la stessa da tempo immemore, non ha molto da amare $\esAi$. Piu' che altro perche' la materia non ha anima, non desidera ne' vivere ne' morire, non e' ne' viva ne' morta. Quindi, certe volte un sasso puo' risultare utile, altre volte inutile, altre un ostacolo. Percio', l'unico caso positivo che sussiste tra $\esAi$ e la Natura e' il caso $2.$. $N$ tuttavia, anche se non ama, non odia. Questo lo aveva osservato lungamente Epicuro e Lucrezio\footnote{De rerum Naturae}. La Natura non e' avversa a noi, semplicemente fa' il suo corso, e a volte e' facile raggiungere con lei un equilibrio, alle altre volte no, o perfino impossibile. \\

Abbiamo detto che un essere e' Dio di se stesso e di chi lo ama. Come fa' una donna od un uomo ad essere Dio, anche se solo di se stesso ? Ma ancora di piu', come facciamo ad essere Dio di noi stessi e di chi ci ama? Sembra strano, ma cio' che provoca la stranezza e' che confondiamo il nostro Io, il nostro essere, con il nostro Ego. Dove con Ego si intende un parte di noi che e' esclusivamente per noi stessi e per nessun'altro. Questa confusione e' dovuta dal nostro egoismo ed egocentrismo, che e' naturale e sano quando siamo bambini\footnote{
    \url{https://en.wikipedia.org/w/index.php?title=Egocentrism\&oldid=998343250}

    \url{https://en.wikipedia.org/w/index.php?title=Narcissism\&oldid=1011686661\#Required\_element\_within\_normal\_development}}, ma che con la crescita e la maturazione diminuisce, e che con lo studio, la preghiera\footnote{pregare=voler amare} e l'impegnarsi ad amare gli altri, puo' diminuire ancora moltissimo. Quando confondiamo noi stessi con il nostro Ego, nelle formule di sopra avviene che la nostra anima elimina dalla sua unione tutti gli altri esseri tranne che lei stessa. Se veramente capiamo quanto e' scritto sopra, e quale e' il senso dell'unione, allora l'Io perde il suo significato piccolo e personale, e diventa una sorgente di acqua di vita per se stesso e per ogni altro. 

E' piu' facile pensare a noi stessi non come Ego, ma come Dio di noi stessi e di chi ci ama, sperimentando e praticando nel tempo la carita' e meditando sull'unione, su come gli altri fanno parte di noi, perfino anche quando il nostro io/ego non ne trae beneficio. Questo sperimentare e praticare e' una cosa che non avra' mai fine, anche da Santi avremo sfide anche difficili con il nostro Ego. Tuttavia, la domanda sara' sempre la stessa: siamo Ego o Amore? Qual'e' la nostra vera natura, la vera gioia di noi stessi?

A volte, non solo e' un osservare e sperimentare una nostra tendenza naturale, ma e' anche una scelta: siamo disposti a cambiare ed accogliere l'altro anche rimpicciolendo il nostro Ego? 

A volte, tale scelta sara' una ``fede'', nel senso di un salto nel buio, che senza poter capire cosa succedera' dopo, faremo, perche' convinti profondamente che comunque la scelta fatta sara' la migliore.

Spero di non essere di turbamento per chi e' credente, ma in questo capitolo formale e scientifico, scritto per chi ha fede e crede nella scienza, ho voluto non aggiungere alcun elemento mistico. Tuttavia, la definizione di sopra di ``Io che ama'' come Dio di se stesso e di chi lo ama, non perde alcun elemento della fede mistica. Infatti, continuando su questa strada, non basta dire ``Io sono Dio''. Dire una tale frase, comporta una grossa, profonda e delicata responsibilita': avere cura della vita propria e di chi ci ama.

Piu' che un'affermazione, allora la domanda da porsi e': amo veramente me stesso, amo veramente chi mi ama, e non solo nel pensiero ma nella mia spontaneita' e nelle profondita' del mio essere ? Sono in pace con la natura, e nella realta' fisica conosco i limiti di essere un uomo (o donna), e mi impegno a stare bene e far stare bene chi mi ama, con tutto me stesso e con tutte le capacita' naturali di chi mi ama ? Sono in pace con la mia vita e spendo la mia vita per la pace di chi mi ama ?

Continuiamo. Se amiamo qualcuno, possiamo declinare tutto il discorso di prima dicendo che \emph{chi amiamo e' Dio per lui/lei stessa, per noi e per chi lo/la ama}. La responsabilita' rimane sempre nostra tuttavia: se amiamo un ragazzo perche' bravo a portare lettere e lo incarichiamo di portare una lettera importante, se sara' perduta per varie circostanze non potremo prendercela con lui. La responsabilita' sara' nostra per averlo amato come corriere. E se il nostro amore e' forte, ci fara' accettare la perdita come un evento naturale voluto da Dio.

Ritornando alla religione, chi e' cristiano ama Gesu' e per lui Gesu' e' Dio. 

Infine, per chi non ci ama, noi non siamo Dio (caso $2.$). E pure, non sara' Dio chi amiamo per chi non lo/la ama.

Di conseguenza, per chi non ama Gesu', egli non e' Dio. Bisogna accettare cio', dato che dare volonta' libera ad ogni uomo e' un atto di amore (o detto equivalentemente, la liberta' degli uomini e' sacra a Gesu').  \\

Nota: viene usata la parola ``desiderio'' in senso psicologico. Se nell'accezione tradizionale e' un qualcosa di parzialmente negativo perche' indica qualche cosa che ancora l'essere non ha, ma a cui ambisce, nell'accezione psicologica e' solo in senso positivo in quanto e' qualche cosa che motiva profondamente l'essere a vivere\footnote{almeno per quanto riguarda i desideri necessari o non superflui}.

Nota: stiamo anche parlando di Anima e non di anima. Nell'accezione tradizionale ``anima'' riguarda l'Io di una persona, e l'Io in genere e' inteso in senso egoistico. Qui con Anima $\anima{A}_i$ indichiamo l'anima $a_i$ come unione di tutte le anime $a_1+a_2+\cdots$ che l'essere $a_i$ ama. Quindi, bisogna fare attenzione, perche' il discorso di sopra non e' un discorso ego-istico, centrato sull'ego di $a_i$, e' un discorso di unita'.
...
\subsection{Prima definizione di Dio}
   ...
   Definiamo Dio, dal punto di vista dell'anima, come qualsiasi colui o colei che soddisfa \emph{completamente} i bisogni e desideri dell'anima. E quando dei bisogni non possono essere soddisfatti, e' colui/colei che mostra, spiega e da' esempio di come questi bisogni non possono essere soddisfatti per il bene maggiore degli altri o del rispetto dei limiti del corpo.

   Non tanto e' la questione se Dio esiste. Ma piuttosto quanto esiste. Un qualsiasi Io che soddisfa al 10\% i bisogni di un anima, sara' per quell'anima Dio al 10\%.

    Se un Io lamenta che Dio non esiste, dovrebbe esaminare se stesso e vedere quanto puo' fare di meglio per l'Anima. La maggior parte delle volte si puo' sempre fare di piu' o diversamente, perche' si e' convinti che gia' si sta' facendo tutto correttamente, ma in realta' non si sta' ascoltando propriamente l'Anima. E se gia' sta' facendo il massimo, sara' facile confortare l'Anima con quel che si potra'.

\subsection{L'animo e il suo limite infinito}
\label{definizioneAnimo}

Abbiamo detto che l'animo di un essere e' quell'entita' che amministra l'energia fisica di ogni sua parte, cioe' di egli stesso e di chi ama, e la spende per proteggerla dal dolore, fisico ed emotico, e per guarirla, e per dare piacere fisico ed emotivo all'Anima.

\def\spirit#1{\textrm{spirit}(#1)}
\def\Animo#1{\spirit{\anima{A}}}

Se l'essere e' $\anima{A}=A_1+A_2+\cdots$, per ogni essere che ama $A_i$, esiste il relativo animo $\spirit{A_i}$. L'unione concorde e coerente di ciascuno rende l'Animo: $\Animo{A} = \spirit{A_1}+\spirit{A_2}+\cdots$.

Un animo essendo il prodotto di processi neurologici, e' limitato ed imperfetto. Tuttavia, l'animo mosso da Amore, \emph{tende} ad un punto fisso, un punto di equilibrio, di matura e piena realizzazione. Ogni anima ha bisogno e desidera costantemente che ogni animo lo raggiunga. Questo punto fisso e' \emph{Dio che si manifesta ed agisce sulla terra} e nel cattolicesimo e' chiamato \emph{Spirito Santo}.

Per rendere l'idea di come e' tendere al Punto Fisso, l'animo vi si avvicina quando nella sua vita non c'e' piu' niente che di lui/lei turba l'Anima, quando coglie e comprende ogni desiderio piu' essenziale e vitale dell'Anima, e riesce a spendersi per far procedere l'anima verso la realizzazione di tale desideri. Che tali desideri non si compino effettivamente nella realta' fisica a causa di forze maggiori e' secondario. Cosi' l'Anima potra' vedere nell'animo se' stessa che ama, ovvero Dio stesso che compie la sua volonta'.

Che l'animo tende ad essere Dio stesso, non vuol dire che diventa super potente, perfetto ed intoccabile, al di sopra della natura, e che non ha bisogno piu' di nessun altro, che e' al di sopra delle leggi, e che puo' fare cio' che vuole di ogni vita. 

Dire che l'animo tende a diventare Dio, vuol dire che tende ad essere speso ed indirizzato verso tutti i bisogni ed i desideri non superflui dell'Anima nella sua completezza e nell'individualita' di ogni sua parte, ovvero dell'anima di ogni essere che la compone. Ogni azione e pensiero che muove, respiro ed intenzione che alimenta, e' volto a creare la vita in ogni essere, a mantenerla ed esaltarla. E tutto questo, nel rispetto dei limiti fisici e psicologici degli esseri che sta' amando, ovvero nel rispetto della Natura fisica e della psiche degli esseri.

Facendo tendere un animo al punto fisso, possiamo adesso definire lo Spirito Santo, e cioe' Dio che dal cielo agisce in terra, come la \emph{chiusura di un qualsiasi animo, o unione di animi, che amano l'Anima}.

Facciamo un esempio. Se pensiamo ad un panetterie che fa' onestamente il suo lavoro, la sua chiusura, il suo punto all'infinito e' un Panetterie, ovvero, un uomo che non solo fa' onestamente e con cura il suo lavoro ma che lo fa' perche' ama se stesso ed ogni suo cliente. Idealmente sa' a chi piace quale tipo di pane e se avesse tempo farebbe un pane personale per ogni suo cliente, adatto in quel giorno alle circostanze che il cliente sta' vivendo. E, non solo, Lui facendo il suo lavoro sta' bene e trova la gioia e la forza di vivere. E, infine, non vive solo per il suo lavoro, ma e' anche un uomo che ama la vita per quello che offre ed ama le persone che gli stanno accanto.

Se nella definizione di pagina \pageref{soulGodDefition} abbiamo definito Dio partendo dal cielo verso la terra, con questa definizione di Dio (Spirito Santo), stiamo andando dalla terra verso il cielo.

Il fatto che l'animo non abbia fisicamente capacita' infinite, non vuol dire che egli non sia manifestazione di Dio. Se quell'animo esiste veramente per l'Anima e direziona le sue energie per servirla e vederla contenta, allora cio' che sta' agendo non e' piu' un animo terreno ma e' Dio.
...
Che un animo muovi un solo piccolo passo verso l'infinito, non e' cosa per niente semplice da conquistare, ed ogni sforzo dell'animo impiegato ad amare l'Anima e' prezioso. Infatti, ogni sforzo vuol dire fisicamente e psicologicamente un consumo del suo corpo e della sua vita.

D'altra parte, pero', solo Dio puo' scegliere un animo per amare l'Anima in un certo tempo, per una certa durata e in un certo luogo e in che modo. Se un animo volesse essere bravo ma Dio non volesse amare tramite lui, allora non nascerebbe amore. E tale cosa non puo' essere forzata: nessun essere, neanche tutti gli esseri insieme, per quanto numerosi e capaci, possono amare l'Anima per loro volonta' egoistica o tramite cose frapposte tra loro stessi ed Ella, come ad esempio, tecnologie, possedimenti, beni e risultati meritevoli. 
...
\subsection{Lo zero}
\label{loZero}
Un'anima puo' venire incontro agli animi, stabilendo che cio' che e' stato gia' raggiunto e' perfetto e non chiedendo piu' di cio' che sta' ricevendo: ``tutto cio' che e', e' Sua volonta'. Ogni cosa e' \emph{gia'} come Lui la vuole''.
