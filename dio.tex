...
\begin{defn}
    ...
    Abbiamo trattato l'Anima, come se fosse una entita' indipendente che agisce, tuttavia, essa e' un'aspetto dello stesso essere. E' l'essere che agisce, che parla di se stesso e degli altri e della Natura, in maniera consapevole ad egli stesso od inconscia.

    L'essere $A_i$ e' Dio di se stesso perche' \emph{definisce} e \emph{vuole} il bene per se' e per gli altri che ama. 
    Definisce il bene perche' lui in quanto anima $\anima{A}_{A_i}$ definisce cio' che e' piacere e cio' che e' dolore. Se e' ugualitario, tutti gli esseri che hanno una definizione uguale alla sua saranno inclusi nell'unione $\anima{A}_{A_i}$ e amati da lui. Se e' caritatevole, anche tutti gli esseri che non sono uguali a lui, ma almeno rispettano, non contravvenendo la sua definizione di essere e di amare, saranno pure inclusi nella sua Anima e amati.

    L'essere vuole il bene, perche' accogliera' ogni forza degli animi volta ad adempiere il suo amore, ovvero a soddisfare i desideri di ogni essere $A_j$ dell'unione $\anima{A}_{A_i}=A_1+A_2+\cdots$.\\

    Dopo aver parlato solo dell'essere $A_i$, sussiste un fatto molto importante: ogni essere esistente $E$, anche quelli non dell'unione $\anima{A}_{A_i}=A_1+A_2+\cdots$, sono Dio di se stessi, perche' cio' che e' stato detto per $A_i$ vale in generale per qualsiasi essere.
    ...
    \begin{itemize}
        ...
        \item    Se uno ama l'altro ma non viceversa, uno stara' nel silenzio e l'altro non lo notera', ma ci sara' una sorta di pace, che tuttavia non e' guerra. Per il primo ci sara' anche la speranza che l'altro possa cambiare e ricambiare il suo amore.
...
%    Abbiamo detto che un essere $A_i$ che rappresenta $\anima{A}$ e' ``confondibile'' con $\anima{A}$. 
%
%    All'Essere che rappresenta perfettamente $\anima{A}$, gli diamo il nome piu' alto: \emph{Dio}. Dio e' amante dell'Anima $\anima{A}=A_1+A_2+\cdots$, in quanto ogni desiderio e bisogno di $\anima{A}$ e' importante e sacro a Lui. Perfino i desideri superflui, che li vede nella loro piccolezza, non li sminuisce.
%
%    Detto piu' semplicemente, possiamo dire che Dio e' quell'essere la cui anima e' esattamente $\anima{A}$, e che lui e solo lui e' l'essere la cui anima e' $\anima{A}$.
%
%    Non solo Lui considera i bisogni e i desideri di $\anima{A}$, ma anche li soddisfa pienamente senza ledere neanche un solo $A_i$ di $\anima{A}$, incluso l'essere $N$, la Natura. Ma di quest'ultimo punto parleremo successivamente quando definiremo il concetto di ``Io''.
%
%
%    Dio non e' terreno, in quanto un essere vivente $A_i$ e' limitato e non puo' amare ogni altro essere $A_j$ perfettamente, in ogni momento della sua vita, per quanto possa eccellentemente avvicinarsi a tale realizzazione. Formalmente, per l'Anima $\anima{A}$ incarnata dall'essere vivente $A_i$ (nota\footnote{che precedentemente abbiamo indicato con $\anima{A}_{A_i}$}), possiamo definire una misura $\sigma_{A_i}$ che per ogni $A_j$ indica quanto veramente e pienamente $A_j$ e' incluso nell'unione $\anima{A}_{A_i}=A_1+A_2+\cdots$. Diciamo che $\sigma_{A_i}(A_j)\in [0,100]$ assume un valore tra zero e cento (ovvero una percentuale). Tanto piu' $\sigma$ e' vicino a 100, tanto piu' $A_j$ e' veramente amato da $A_i$, nei suoi limiti psico-fisici, e tanto piu' infinitamente e' difficile e intensamente piacevole per $A_i$ amare $A_j$ (nota \footnote{si potrebbe sostituire ``100'' con $+\infty$, a seconda se si vuole sottolineare che un essere umano non e' Dio, o che Dio e' divino. Le due definizioni sono equivalenti: se chiamiamo con $x$ la sigma in $[0,1]$ (non usiamo $[0,100]$ per semplificare le formule, ma la correttezza del discorso vale) e con $y$ la sigma in $[0,\infty[$, con la biezione continua $x=\frac{y}{y+1}$ si puo' passare da una all'altra. L'inversa e' $y=\frac{-x}{x-1}$}). 
%
%    Qui, cosi' stiamo introducendo un altro concetto: un essere $A_i$ puo' non amare pienamente $A_j$, anche se la sua anima, nella sua forma piu' completa e compiuta, vivrebbe un benessere piu' profondo ed autentico nel farlo.
%
%    Quello Gesu' diceva agli uomini, e' che amando il prossimo, avrebbero vissuto meglio, perche' la loro anima, nell'evolversi in tale direzione, avrebbe trovato una realizzazione piu' profonda, vera ed autentica.
%
%    Ricapitolando, Dio e' un essere $D$ tale che $\sigma_{D}(A_j)=100$ per ogni $A_j$, in ogni momento presente, passato e futuro. Possiamo inoltre definire l'Anima $\anima{A}$ come l'anima universale: $\anima{A} = \anima{A}_D$.\\
%
%
%    Infine, Dio e' reale: un essere vivente incarna Dio per una certa approssimazione, i Santi in ottima approssimazione. Ed anche perche' $\anima{A}$ e' la realizzazione piu' sana, forte ed autentica di $\anima{A}_{A_i}$, per ogni $A_i$ vivente. Tutti i suoi bisogni e desideri sono veri, necessari e non superflui.
%
\end{defn}

    I desideri in misura forte o debole di Dio sono i desideri veri ed autentici di $\anima{A}$ tutta, ed incluso il piu' piccoli degli esseri $A_i$.
    I desideri di Dio non sono desideri umani, essi sono tutti appagati. Dio non soffre nel desiderare e cio' che desidera e'.

%    Logicamente, la cosa e' chiara, infatti, un essere i cui desideri sono esattamente i desideri di $\anima{A}$, e quindi i desideri di ogni $A_i$ e di $N$, non va' in conflitto con alcuno e con nessun cosa nell'universo naturale. E' in armonia con tutti e con la Natura. Se includiamo in $\anima{A}$ tutti gli esseri viventi e la natura, allora ogni cosa che accade e' dovuta o ad un essere o ad un fenomeno della natura $N$, quindi, possiamo affermare che ogni cosa che accade e' Sua volonta', e che Lui e' in Pace con la Sua stessa volonta'.\\
%
%La definizione formale di Dio data sopra in \ref{soulGodDefition}, riguarda i desideri dell'Anima e non parla del soddisfacimento di tali desideri. Dio oltre ad essere colui che alimenta i desideri naturali nostri e di tutti gli altri, e' anche l'essere che soddisfa pienamente tali desideri. Piu' in seguito (vedi \ref{definizioneIo} pag. \pageref{definizioneIo}), definiremo l'Io. Un Io che agisce in terra per Suo amore, e' cio' che nel concreto soddisfa, parzialmente, i desideri dell'Anima, per quanto puo' e gli e' concesso. L'anima che si arrende ai limiti di tutti gli Io che la amano, poi, completera' l'equazione dell'esistenza concreta di Dio: un'anima $\anima{A}_{A_i}$ che tende a diventare $\anima{A}$ e che gioisce della carita', fisicamente limitata, degli Io che la amano, vive la Pace e la Gioia di Dio, e fa' esperienza del paradiso in terra. Un Io che agisce per conto di Dio, fa' esperienza della sicurezza e forza di Dio, che tutto muove e ogni essere nutre e arricchisce.
...
Dire che l'Io tende a diventare Dio, non e' detto per caso. La tensione e' continua, ma mai finita, dato che un uomo non puo' essere pienamente divino. Nella dottrina Cattolica, solo Gesu' lo e' stato, ma di questo non parleremo in questo libro.
